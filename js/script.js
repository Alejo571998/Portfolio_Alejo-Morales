...
// Cada feature corre aislada: si una falla, no rompe las demás
// ni deja contenido oculto (ver .reveal en estilos.css).
// ============================================================
(() => {
  'use strict';

  // Confirma que el JS cargó y corrió: recién ahí el CSS activa
  // el efecto de aparición en scroll. Si este archivo no carga,
  // todo el contenido queda visible igual (progressive enhancement).
  document.documentElement.classList.add('js');

  const safe = (name, fn) => {
    try { fn(); } catch (err) { console.warn(`[portfolio:${name}]`, err); }
  };
  ...
  safe('cursor', () => {
    ...
    if (!canHover || !cursorDot || !cursorRing || reduceMotion) return;
